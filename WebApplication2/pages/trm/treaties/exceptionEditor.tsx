...
interface EditorProps {
    ...
    validation: TreatyExceptionDtoValidator;
    onChange: { (m: Dtos.TreatyExceptionDto): void };
    onSave: { (): void };
    onCancel: { (): void };
}
...
export class ExceptionEditor extends React.Component<EditorProps, {}>
{
    ...
    private innerContent() {
        ...
        var exceptionTypes = this.props.treatyExceptionTypes.map(x => x.map(y => { return { id: y.value, name: y.label }; }));

        var combined = Framework.Pending.combine(entityTypesLookup, this.props.stockTypes, exceptionTypes, (entityTypes, stockTypes, exceptionTypes) => { return { entityTypes, stockTypes, exceptionTypes }; });
        ...
        return <div className="adr-popup-section container-fluid ps-0">
            {
                Framework.Loader.for(combined, data => 
                    new Framework.FormBuilder(this.props.exception)
                        ...
                        .addDropdown("Exception Type", data.exceptionTypes, m => data.exceptionTypes.find(x => x.id === m.exceptionType), (m, v) => { m.exceptionType = v && v.id; m.exceptionTypeName = v && v.name }, "ExceptionType", val.exceptionType)
