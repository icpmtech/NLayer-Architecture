...
export class RequestNewsReport extends React.Component<Props, State> {
    ...
    private onExportClick = (startDate) => {
        ...
        let method = api.queueLatestNewsExport({ startDate: this.state.startDate });
    ...
    private renderText() {
        return (
            <div>
                ...
                {!this.state.exportSuccess ? <div style={{ width: '100%' }}><Components.DateInput value={this.state.startDate} onChange={(d) => this.setState({ startDate: d })} qa="ReportGenerationStartDate"/></div> : null}
                ...
                <p>To download the exported files go to the <a href="/reports/export">Reports</a> page.<br /><br /></p>
            </div>
        );
    }
