...
export class ExportRpaPopup extends React.Component<PageProps, PageState> {
    ...
    private onExportClick = () => {
        connect(new Apis.ReportsApi().queueRpaReport({ roundId: this.props.roundId }), null, response => {
    ...
    private renderError() {
        return this.state.exportError ? <Message type="alert" message={"Error exporting round RPA"} qa="ErrorExportingRoundRpaMessage"/> : null;
    }

    private renderSuccess() {
        return this.state.exportSuccess ? <Message type="success" message={"Successfully started export process"} onClose={this.props.onClose} qa="SuccessfullyStartedExportProcessMessage"/> : null;
    }
